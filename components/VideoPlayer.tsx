...
export default function VideoPlayer({ src, isLive = false, poster }: VideoPlayerProps) {
  ...
  useEffect(() => {
    ...
    const onTimeUpdate = () => {
      ...
      if (!isLive) setProgress((video.currentTime / video.duration) * 100);
    };
      ...
      <div className={`absolute inset-0 bg-gradient-to-t from-black/80 via-transparent to-transparent transition-opacity duration-300 ${showControls ? "opacity-100" : "opacity-0"}`}>
        {!isPlaying && !isLoading && (
          <button onClick={togglePlay} className="absolute inset-0 flex items-center justify-center">
            <div className="w-16 h-16 rounded-full bg-accent/20 backdrop-blur-md flex items-center justify-center border border-accent/30 hover:bg-accent/30 transition-all hover:scale-105">
        ...
        <div className="absolute bottom-0 left-0 right-0 p-4">
          {!isLive && (
            <div className="mb-3">
              <input
                ...
                style={{ background: `linear-gradient(to right, #00FF88 ${progress}%, rgba(255,255,255,0.15) ${progress}%)` }}
              />
            </div>
          )}
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              {!isLive && <span className="text-xs text-white/60">{formatTime(currentTime)} / {formatTime(duration)}</span>}
              {isLive && (
